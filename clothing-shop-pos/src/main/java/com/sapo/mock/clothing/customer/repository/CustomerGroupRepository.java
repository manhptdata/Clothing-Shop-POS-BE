...
public interface CustomerGroupRepository extends JpaRepository<CustomerGroup, Integer> {
    ...
    @Query("SELECT new com.sapo.mock.clothing.customer.dto.response.CustomerGroupResponse(" +
            "g.id, g.name, g.description, g.status, COUNT(c), g.note, g.minSpending, g.maxSpending, g.code, g.createdAt) " +
            ...
            "GROUP BY g.id, g.name, g.description, g.status, g.note, g.minSpending, g.maxSpending, g.code, g.createdAt")
